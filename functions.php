...
function nosidebar_shortcode( $atts ){
 return '<style type="text/css"> #sidebar { display: none; } </style>';
}
add_shortcode( 'nosidebar', 'nosidebar_shortcode' );
...
function banner_shortcode( $atts, $content = null ) {
   ...
   return '<div class="banner">' .do_shortcode($content). '<br style="clear:left;" /></div>';
} 
add_shortcode( 'banner', 'banner_shortcode' );

function banner_image_shortcode( $atts ) {  
	extract( shortcode_atts( array(
			'img' => 'http://farm9.staticflickr.com/8018/6965889082_161ac080dc_c.jpg',
			'caption' => '',
			'link' => '#',
			'valign' => 'center',
			'halign' => 'center',
		...
		if (!empty($caption)) {
			return '<a href="'.$link.'" style="background-image:url('.$img.'); background-position: '.$halign.' '.$valign.'"><span class="caption">'.$caption.'</span></a>'; 
		}
		else {
			return '<a href="'.$link.'" style="background-image:url('.$img.'); background-position: '.$halign.' '.$valign.'"></a>'; 
		}
}  
